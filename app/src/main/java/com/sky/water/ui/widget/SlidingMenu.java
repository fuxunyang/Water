...
public class SlidingMenu extends HorizontalScrollView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (once) {//执行一次
            ...
            menuWidth = menu.getLayoutParams().width = screenWidth / 5 * 2;
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN://获取按下时的信息
                ...
                if (isOpen() && downX > menuWidth) {
                    return true;//打开时，某些部分的touch事件不向下传递
                }
                break;
            case MotionEvent.ACTION_MOVE:
                if (downY > screenHeight * 2 / 5 && (downX - ev.getRawX()) < -100) {
                    return true;
                }
                break;
        }
